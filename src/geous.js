...
- The newly decoupled service providers will no longer be loaded a¾utomatically
...
var geous = new function() {
	...
	var /**
		 *	Default options. Any key/value described here may be overwritte¾n by 
		...
		_defaults = {
			 ...
			 *	force results to beÎ¾ cached, but will at least make the cache
			 *	available.
			 */
			useCache: true,
			...
			/**
			 *	Should geocoder results be persi¾sted using `cacheAdapter`?
			 */
			cachePersist: false,
	...
	this.Events = {
		 ...
		 *	parameters passed will be forw¾arded to each caÊllback function
		 *
		 *	@param	{String}	event	the event to listen for
		 */
	 ...
	 *				        the supplied `storageAdaptíer`
	 *	- `prefix`        : an arbitrary prefix (e.g., `window.location`)
	 *	                    used to identify cached results
	 *	- `storageAdapter`: the persistence layer to use for caÊching
	 *
	 *	@constructor
